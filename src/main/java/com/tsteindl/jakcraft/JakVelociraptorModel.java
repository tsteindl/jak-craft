...
public class JakVelociraptorModel extends TDEEntityModel<EntityJakVelociraptor> {
  ...
  public void animate(Animator animator, EntityJakVelociraptor entity, EntityRenderData entityRenderData) {
    ...
    float neckYaw = bodyRotDeviation * 2.0F + entityRenderData.entityYaw
        + ((Double) entity.clientNeckRot.get(entityRenderData.partialTick)).floatValue();
    animator.getTransform("neck").setLookAt(this.clamp(neckYaw, -60.0F, 60.0F),
        this.clamp(entityRenderData.entityPitch, -20.0F, 20.0F));
    ...
    animator.setAnimation(animation,
        AnimationBlending.create(InterpolationType.CATMULLROM, EasingType.NONE, 0.15000000596046448), 1.0, 1);
    if (entity.actionController.is(entity.actionRoar) && !entity.isBaby() && entity.actionController.getTicks() > 5
        && entity.actionController.getTicks() < 40) {
      this.renderRoarParticles(entity, this, entityRenderData, "head", 0.5F, 0.5F, 2);
    }

    if (entity.actionController.is(entity.actionEat) && entity.actionController.getTicks() > 30
        && entity.actionController.getTicks() < 50) {
      this.renderFoodParticles(entity, this, entityRenderData, new AVector3f(0.0F, -0.3F, -0.2F),
          new AVector3f(0.4F, 0.5F, 0.4F), 5, "head");
    }

  }

  public void renderPassenger(EntityJakVelociraptor entity, LivingEntity passenger) {
  ...
  public ResourceLocation getTexture(EntityJakVelociraptor entity) {
    if (entity.isBaby()) {
      return TEXTURE_BABY;
    } else {
  ...
  public static TDELivingRenderer<EntityJakVelociraptor> createRenderer(EntityRendererProvider.Context context) {
    JakVelociraptorModel model = new JakVelociraptorModel(
        modelLocation("ceratosaurus", "geo.json"), animationsLocation("ceratosaurus", "animation.json"));
    ...
    JakVelociraptorModel babyModel = new JakVelociraptorModel(
        modelLocation("ceratosaurus", "geo_baby.json"), animationsLocation("ceratosaurus", "animation_baby.json"));
